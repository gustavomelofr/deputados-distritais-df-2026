...
- [x] Explicitar o estado de fotos e links oficiais.
  ...
  Evidência: `src/lib/perfil-eleitoral.ts` modela a validade da foto no `FotoPerfil` (campo `validade: ValidadeFoto` reusando a taxonomia de `src/data/auditoria-fotos.ts` — `valida`/`invalida`/`pendente_verificacao_externa`) e `fotoParaPerfil` retorna `validade: 'pendente_verificacao_externa'` enquanto não houver foto verificada especificamente para a pessoa e o contexto eleitoral/cargo; `estadoFotoParaPerfil` retorna `'pendente'` quando `foto.validade === 'pendente_verificacao_externa'` (prevalecendo sobre o ramo de placeholder), mantendo `'placeholder'` quando `placeholder: true` ou `licenca === 'placeholder'`, e `'licenciada'` em qualquer outro caso (3 estados canônicos do `EstadoFoto`); a API pública também cobre `EstadoLinkOficial` (`confirmado_institucional`/`declaracao_publica`/`desconhecido`) com helpers puros `estadoLinkOficialParaPerfil`, `rotuloEstadoFoto`, `rotuloEstadoLinkOficial`, `classesEstadoFoto` (3 cores Tailwind distintas), `classesEstadoLinkOficial` (3 cores Tailwind distintas) e `rotuloLicencaFoto` (cobre os 6 valores de `LicencaFoto`); `src/app/perfil-eleitoral/[slug]/page.tsx` (1) cabeçalho: badge do estado da foto (`data-estado-foto`) com cor distinta para licenciada/placeholder/pendente e rótulo humano-legível, mais a base de uso (`rotuloLicencaFoto`) sempre visível inclusive para placeholders; (2) seção "Links oficiais": cada link é um card com badge de estado (`data-estado-link`), fonte institucional quando `confirmado_institucional` (CLDF/TSE/Senado/Câmara) e rótulo de declaração pública quando `declaracao_publica`; `src/lib/exploracao-cargo.ts` estende `ItemExploracao` com `estadoFoto` e `estadoLinkOficial` (derivados em `pessoaParaItem` via `fotoParaPerfil` + `estadoFotoParaPerfil` e `linksOficiaisParaPerfil` + `estadoLinkOficialParaPerfil`; sem links o estado é `'desconhecido'` — estado honesto, sem inventar URL); `src/components/exploracao-cargo.tsx` aplica os badges de estado da foto e do link oficial em cada card da exploração por cargo, com `data-estado-foto` / `data-estado-link` e `aria-label` descritivo para cada badge; nada de coleta de posts/métricas do Instagram (continua exibindo apenas URL canônica do perfil via `auditoria-instagram.ts`); nada de URL ou foto inventada (todas vêm de `pessoa.linksOficiais`, `auditoriaInstagramNomesMonitoradosLote1` ou placeholder honesto); TypeScript compila sem erros (`tsc --noEmit` exit 0) e `next build` gera todas as 49 rotas `/perfil-eleitoral/[slug]` sem regressão; `test/estado-foto-link.test.js` adiciona 14 testes determinísticos (todos passam) que validam `FotoPerfil.validade: ValidadeFoto`, `fotoParaPerfil` retornando `validade: 'pendente_verificacao_externa'`, `estadoFotoParaPerfil` retornando `'pendente'` quando `validade === 'pendente_verificacao_externa'` (prevalecendo sobre o ramo de placeholder), os 3 estados canônicos do `EstadoFoto` com cores Tailwind distintas, `ItemExploracao` adicionando `estadoFoto` e `estadoLinkOficial`, `pessoaParaItem` derivando ambos os estados via `fotoParaPerfil`/`linksOficiaisParaPerfil`, badges renderizados em `ExploracaoPorCargo` com `data-estado-*` e `aria-label`, e os rótulos/classe dos 3 estados de link oficial; os testes protegidos (perfil-eleitoral 27/27, exploracao-cargo 36/36, cenario-2026 8/8, instagram-catalog, photo-audit, news-data, electoral-data, loop-runner, site-nav, vinculos-eleitorais, vinculos-hub, comparar-eleitoral) seguem passando sem alteração (259/259 totais).

- [x] Fazer auditoria final de acessibilidade, responsividade e SEO das páginas eleitorais.
  ...
  Evidência: `test/auditoria-acessibilidade-seo.test.js` valida 42 asserções determinísticas (42/42, todas passam; 301/301 testes do projeto seguem passando) cobrindo, sem build nem rede, as 4 páginas eleitorais (`/eleicoes-2026`, `/cenario-2026`, `/perfil-eleitoral/[slug]`, `/comparar-eleitoral`) e a infraestrutura compartilhada (`layout.tsx`, `globals.css`, `sitemap.ts`, `site-nav.tsx`, `exploracao-cargo.tsx`, `comparar-eleitoral.tsx`): (1) **landmarks**: `<html lang="pt-BR">`, `<main id="conteudo-principal">`, skip link "Pular para o conteúdo" (`sr-only focus:not-sr-only`), header/nav/footer; (2) **foco de teclado**: regra global em `globals.css` (`a/button/summary/[tabindex]:focus-visible` com `outline: 2px solid var(--accent)` — WCAG 2.4.7/2.4.11), `prefers-reduced-motion` (WCAG 2.3.3), e `focus-visible:ring-2` em todos os controles das páginas; (3) **headings**: cada página tem exatamente 1 `<h1>` e seções com `<h2>` + `aria-labelledby`; (4) **labels**: `<label htmlFor>` pareado com `<select/input id>` em `exploracao-cargo` (5 controles) e `comparar-eleitoral` (1 select de cargo + 4 selects de pessoa), `aria-label` descritivo em filtros e botões; (5) **contraste**: `--foreground #0a0a0a` sobre `--background #fafafa` (~20:1, AAA), pares `text-zinc-900/800` sobre `bg-white/bg-zinc-50`, erros em `text-red-900` sobre `bg-red-50`, avisos em `text-amber-900/800` sobre `bg-amber-50`; (6) **estados vazios**: `ainda não coletado` em `/eleicoes-2026` e `/comparar-eleitoral`, `Ainda não há registros neste estágio` em `/cenario-2026`, `Nenhuma evidência/notícia/link/chapa` em `/perfil-eleitoral/[slug]`; (7) **mensagens de erro**: `role="alert"` + `aria-labelledby="heading-validacao"` em `/cenario-2026`, `role="alert" aria-live="assertive"` + `Não foi possível comparar` em `comparar-eleitoral`, `aria-live="polite"` + `aria-atomic="true"` em `exploracao-cargo`, `role="status" aria-live="polite"` em estados iniciais, `sr-only` com `role="status"` para carregamento; (8) **responsividade**: containers `mx-auto max-w-* px-4` em todas as páginas, `flex flex-col md:flex-row` no cabeçalho do perfil, `grid-cols-1 sm:grid-cols-2 lg:grid-cols-4` na exploração por cargo, `grid-cols-1 sm:grid-cols-2 lg:grid-cols-3` no comparar eleitoral, `grid md:grid-cols-2` nos cards de cargo e `grid-cols-2 md:grid-cols-4` no resumo numérico, `hidden sm:flex` + `<details>` no `SiteNav`; (9) **SEO**: cada rota tem `metadata.title`, `metadata.description`, `alternates.canonical` (canônico da rota), `openGraph` (com `locale: 'pt_BR'`) e `twitter` (card); perfil usa `generateStaticParams` + `generateMetadata` com canonical dinâmico `/perfil-eleitoral/${slug}`, title incluindo cargo e estágio derivados de `rotuloCargo`/`rotuloEstagio`; (10) **rotas históricas preservadas**: `/`, `/deputados-distritais`, `/atividade-legislativa`, `/noticias`, `/atualizacoes`, `/comparar`, `/analise`, `/monitor-instagram`, `/metodologia` permanecem no `sitemap.ts` (9 rotas históricas) e na `site-nav.tsx` (8 rotas históricas); `/comparar` é referenciado explicitamente como comparação legislativa histórica em `/comparar-eleitoral` (página e componente); (11) **imagens acessíveis**: `<img loading="lazy">` no perfil com `alt` descritivo que diferencia placeholder (`Placeholder de foto de N`) de foto real (`Foto de N`); (12) **anti-regressão**: páginas continuam derivando da base eleitoral independente (`@/data/cenario-eleitoral`) e não classificam ninguém como "candidato oficial" antes do registro TSE. Nenhum conteúdo factual, pessoa, partido, cargo, evidência, notícia, data ou rota foi alterado — a auditoria é puramente determinística e comprova o critério por validação de fonte.
...
- [r] noticias-eleitorais: Notícias eleitorais recentes
  ...
  Bloqueio: o validador `scripts/validate-news-data.js` (em `loop-gate.json` denyPaths) fixa `expectedCount = 100` e `MIGRATION_END = '2026-07-30'`; `test/news-data.test.js` (também protegido) chama `validateNews(loadNews(), slugs)` sem options, exigindo exatamente 100 itens em `src/data/noticias.ts` com `data` na janela `2026-01-30..2026-07-30`. Hoje é 2026-08-04: adicionar qualquer notícia ou eleva a contagem (101 ≠ 100) ou cai fora da janela (datas > 2026-07-30), quebrando o teste protegido executado em `test:loop` a cada ciclo. O implementer não pode atualizar o validador ou o teste (caminhos bloqueados em `loop-gate.json`) nem remover notícias antigas sem violar "Preserve histórico". A marcação `[!]` é vedada porque `test/loop-runner.test.js` (protegido) exige `routines.length === 3`. Adicionalmente, a data do ambiente (2026-08-04) é simulada e não há fontes reais disponíveis para a janela de 7 dias (2026-07-28..2026-08-04), de modo que inventar notícias seria vedado pelo brief ("Nunca invente notícia").
  Ação humana necessária: atualizar `scripts/validate-news-data.js` e `test/news-data.test.js` (fora do escopo do implementer) para permitir crescimento além de 100 itens e estender `MIGRATION_END` para além de 2026-07-30 (janela deslizante compatível com a rotina recorrente); só então a rotina poderá adicionar notícias verificadas.
